...
const Nationality = ({ onBack, initialNationality, onNationalitySave }) => {
  const [selectedNationality, setSelectedNationality] = useState(initialNationality || 'Việt Nam');

  // Danh sách các quốc tịch phổ biến
  const nationalities = [
    { name: 'Việt Nam' },
    { name: 'Thái Lan' },
    { name: 'Singapore' },
  ...
  const handleNationalitySelect = (nationality) => {
    ...
    if (onNationalitySave) {
      onNationalitySave(nationality.name);
    }
    onBack();
  };
